function setupGui() {
    ...
    droneFolder.add(config, 'mutationRate').min(0.001).max(1).step(1);
    ...
    statsFolder.add(stats, 'generationCounter').name('Generations').listen();
    statsFolder.add(stats, 'droneCounter').name('Active Drones').listen();
    statsFolder.add(stats, 'averageAge').name('Avg. Drone Age').listen();
    statsFolder.add(stats, 'highestAge').name('Oldest Drone').listen();
    statsFolder.open();
